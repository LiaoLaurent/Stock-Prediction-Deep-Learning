...
def compute_order_book_features(
    ...
    lob_data = df[lob_columns].resample(resample_rate).last()
    order_book_data = order_book_data.merge(lob_data, left_index=True, right_index=True, how="left")
...
def process_and_combine_data(
    ...
    existing_data_paths = [path for path in file_paths if os.path.exists(path)]
    daily_data_list = []

    # Process each day's data
    for file_path in existing_data_paths:
